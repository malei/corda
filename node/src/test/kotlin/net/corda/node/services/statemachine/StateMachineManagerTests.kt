...
class StateMachineManagerTests {
    ...
    fun `exception while fiber suspended`() {
        ...
        assertThat(uncaughtException).isSameAs(exceptionDuringSuspend)
    }
    ...
    fun `exception thrown on other side`() {
        node2.services.registerFlowInitiator(ReceiveFlow::class) { ExceptionFlow }
        val future = node1.services.startFlow(ReceiveFlow(node2.info.legalIdentity)).resultFuture
        net.runNetwork()
